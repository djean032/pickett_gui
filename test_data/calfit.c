...
#define FMT_xblnIR "%14.7f %10.7f %6.4f\n"
  ...
  fprintf (lufit, "LINES REQUESTED=%5d NUMBER OF PARAMETERS=%3d", limlin,
           npar);
  ...
  ndfree0 = 0;
  if (inpcor != 0)
    { /* initialize fit from supplied variance */
      pvar = var;
      pfit = fit;
      for (n = 1; n <= nfit; ++n)
        { /* copy from var */
    ...
    { /* initialize fit when there is no supplied variance */
      pvar = var;
      pfitb = fitbgn;
      fitbgn[0] = 1. / var[0];
      var[0] = 0.;
  ...
  do
    {
      ...
      do
        {
          ...
          if (iblk != lblk)
            { /*  get size of block */
              ...
              if (nsize > maxdm)
                {
                  printf ("WARNING .. SIZE OF BLOCK %d2 IS %d AND EXCEEDS "
                          "DIMENSIONS\n",
                          iblk, nsize);
                  exit (EXIT_FAILURE);
                }
      ...
      ex = 1.;
      do
        { /*   form least squares matrix, LOOP over lines */
          ...
          xwid = 1.;
          do
            { /*    UNTIL all elements of blend are found */
          ...
          if (iflg == 1)
            {
              ...
              if (xerr < 0.)
                {
                  fprintf (lufit, FMT_xbgnIR, line, aqnum, xfrq, afrq, adif,
                           xerr, cerr);
                }
              else
                {
                  fprintf (lufit, FMT_xbgnMW, line, aqnum, xfrq, afrq, adif,
                           xerr, cerr);
                }
              fputc ('\n', lufit);
              lblnd = line + 1;
          ...
          else
            {
              ...
              cwid = 0.;
              do
                { /* UNTIL all elements of blend are printed */
                  i = frqdat (lblnd, &iblnd, &xfrq, &xwt, &xerr, qnum);
                  if ((iblnd & 1) != 0)
                    {
                      xfrq = 0.;
                      ...
                      if (xerr < 0.)
                        {
                          fprintf (lufit, FMT_xbgnIR, lblnd, aqnum, xfrq, frq,
                                   frq, xwid, xfrq);
                        }
                      else
                        {
                          fprintf (lufit, FMT_xbgnMW, lblnd, aqnum, xfrq, frq,
                                   frq, xwid, xfrq);
                        }
                      fputc ('\n', lufit);
                    }
      ...
      marqlast = marqp[0];
      marqflg = lsqfit (fit, ndfit, nfit, 1, marqp, varv, oldfit, erpar, dpar,
                        iperm);
      if (marqflg != 0)
        {
  ...
  fprintf (lupar, "%4d %4d %4d %4d %14.4E %14.4E %14.4E %12.10f\n", npar,
           limlin, nitr, nxpar, marqlast, xerrmx, parfac0, fqfacq);
  fprintf (luvar, "%4d %4d %4d %4d %14.4E %14.4E %14.4E %12.10f\n", npar,
           limlin, nitr, nxpar, marqlast, xerrmx, parfac0, fqfacq);
  for (icnt = 0; icnt < noptn; ++icnt)
    { /*  save option lines */
  ...
  while (k > 0)
    { /* copy correlation lines or extra parameter lines */
      k = fgetstr (card, NDCARD, lubak);
      if (k > 0)
        {
          card[k] = '\n';
...
int
qnfmt2 (int nqn, short *qnum, char *aqnum)
{
  /* Local variables */
  int i;
...
int
parer (double par, double errx, double dif, char *ptmp)
{
...
int
linein (FILE *luin, int *nline, int iqnfmt)
{
  /* Local variables */
  SXLINE *xline;
...
int
lineix (FILE *lu, int flg, int nline, int nblkpf, int iqnfmt)
      ...
      if (iblku == 0 && (xline->bln & 1) == 0)
      ...
      else
        {
          ...
          if (flg < 0)
            {
              iqnum = xline->qn;
              fprintf (lu, " %4d%4d%4d%4d%4d:", nread, iblku, indxu, iblkl,
                       indxl);
              for (i = 0; i < ncat; ++i)
                {
                  j = iqnum[i];
