...
int
getlbl (int npar, bcd_t *idpar, char *parlbl, char *fil, int idiv, int lblen)
{
#define NDEC 10
#define NCARD 130
...
int
getblk (int *iblk, int *indx, short *iqnum, int nblkpf, int ipos, int nqn)
{
...
int
prcorr (FILE *lufit, int nfit, double *cor, int ndcor, double *err)
...
void
dnuadd (int npar, int nparx, int initl, int indx, int ifac, double *egy,
        double *egyder, int nsize, int line, double *par, double *fac)
{ /*     subroutine to add energies and derivatives to
     frequency lists */
  /*  ON INPUT: */
  ...
  /* add derivatives */
  if (indx < 0)
    { /* set up to ignore last few derivatives */
      indx = -1 - indx;
      nparn = nparx;
      noff = npar - nparn;
      itmp = nparn;
...
double
dnuget (int iflg, int npar, double f, int line, double *dvec)
{
  SXLINE *xline;
  double frq;
...
int
getdbk (int *link, int *iblk, int *indx, int *initl, int *ifac)
{
  SXLINE *xline;
...
int
frqdat (int line, int *ibln, double *txfrq, double *txwt, double *txerr,
        short *iqn)
{ /*  gets blend code, frequency, weight, error , and
     quantum numbers */
...
int
lnlink (int *prvblk, int nblk, int iblk, int line)
{
  int i, last;
  SXLINE *xline;
