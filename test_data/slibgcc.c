...
int
chtime (char str[], const int N)
{ /* subroutine to return 24 characters with
     the time and date */
#define NCTIME 24
  char *buffer;
  time_t curtime;
...
int
filget (const int argc, char *argv[], const int nfile, char *cfil[],
        const char *cext[])
{
#define NSTR 82
  char str[NSTR];
...
FILE *
fopenq (const char *fname, const char *opt)
{
  FILE *hret;
  char *fnew, *path;
  if (opt[0] == 'l')
...
int
fgetstr (char *buffer, const int N, FILE *stream)
